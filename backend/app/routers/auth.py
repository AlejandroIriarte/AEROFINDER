...
async def register(
    ...
    """
    Registra un nuevo usuario familiar.
    - Verifica email único
    - Obtiene el rol 'familiar' del catálogo
    - Crea el usuario con is_active=TRUE (listo para login immediatamente)
    - Crea registro en user_sessions si es exitoso
    """
    ...
    try:
        ...
        if existing.scalar_one_or_none() is not None:
            raise HTTPException(
                ...
                detail="El correo ya está registrado. Inicia sesión.",
            )
    except HTTPException:
        raise
    except Exception:
        logger.error("Error al verificar email durante registro email=%s", body.email, exc_info=True)
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Error interno")
    ...
    try:
        ...
        logger.error("Error al buscar rol familiar durante registro", exc_info=True)
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Error interno")
    ...
    try:
        ...
        logger.info("Nuevo usuario registrado email=%s user_id=%s", body.email, new_user.id)

    except Exception:
        logger.error("Error al crear usuario durante registro email=%s", body.email, exc_info=True)
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Error interno")
