...
from app.models.enums import MissingPersonStatus, RelativeRelation, RoleName
...
# ── Endpoints de Admin: Gestión de Casos Pendientes ──────────────────────────
# IMPORTANTE: deben ir ANTES de /{person_id} para que FastAPI no los capture
# como si "pending-review" o "report" fueran UUIDs.

@router.get("/pending-review", response_model=list[PersonResponse])
async def list_pending_persons(
    ...
    """
    Lista casos en pending_review para que admin o ayudante los aprueben.
    Solo acceso: admin, ayudante.
    """
    try:
        result = await db.execute(
            select(MissingPerson)
            .where(MissingPerson.status == MissingPersonStatus.pending_review)
            .offset(skip)
            .limit(limit)
        )
        persons = result.scalars().all()
    except Exception:
        logger.error("Error al listar casos pendientes", exc_info=True)
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Error interno")
...
# ── Endpoints de Familiar: Reportar Casos ────────────────────────────────────

@router.post("/report", response_model=PersonResponse, status_code=status.HTTP_201_CREATED)
async def report_person_familiar(
    body: PersonReportCreate,
    current_user: CurrentUser = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
) -> PersonResponse:
    """
    Endpoint para que un usuario autenticado (familiar, ayudante, etc.) reporte un caso.
    El caso se crea en pending_review y queda a revisión del admin.
    El usuario reportante queda vinculado como PersonRelative.
    """
    if current_user.role == RoleName.admin or current_user.role == RoleName.buscador:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Los operadores deben usar POST /persons en su lugar",
        )
    ...
    try:
        ...
        # Vincular al usuario como familiar
        relative = PersonRelative(
            user_id=current_user.id,
            missing_person_id=person.id,
            relation=RelativeRelation.other,  # Por defecto
            verified=False,
        )
        db.add(relative)
        await db.flush()
        ...
        logger.info(
            "Caso reportado por familiar user_id=%s person_id=%s",
            current_user.id,
            person.id,
        )

    except Exception:
        logger.error("Error al reportar caso por familiar", exc_info=True)
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Error interno")
...
async def update_person_status(
    ...
    """
    Cambia el status de una persona (approving pending_review → active/rejected, etc).
    Solo admin o ayudante.
    Validaciones:
    - pending_review → active (aceptar caso)
    - pending_review → rejected (rechazar caso)
    - active → found / deceased / unresolved_archived (cerrar búsqueda)
    """
    try:
        result = await db.execute(
            select(MissingPerson).where(MissingPerson.id == person_id)
        ...
        logger.error("Error al buscar persona para cambiar status id=%s", person_id, exc_info=True)
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Error interno")
    ...
    allowed_transitions = {
        MissingPersonStatus.pending_review: [MissingPersonStatus.active, MissingPersonStatus.false_report],
        MissingPersonStatus.active: [MissingPersonStatus.found_alive, MissingPersonStatus.found_deceased, MissingPersonStatus.archived],
        MissingPersonStatus.false_report: [],
        MissingPersonStatus.found_alive: [],
        MissingPersonStatus.found_deceased: [],
        MissingPersonStatus.archived: [],
    }

    if new_status not in allowed_transitions.get(current_status, []):
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail=f"Transición no permitida: {current_status} → {new_status}",
        )

    person.status = new_status
    logger.info(
        "Status actualizado pessoa_id=%s: %s → %s por user_id=%s",
        person_id,
        current_status,
        new_status,
        current_user.id,
