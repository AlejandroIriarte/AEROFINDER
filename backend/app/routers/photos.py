...
async def request_photo_upload_url(
    ...
    try:
        file_record = File(
            ...
            bucket=settings.minio_bucket_photos,
            object_key=object_key,
            sha256_hash=f"pending-{photo_id}",  # único por foto; se reemplaza en /confirm
            size_bytes=1,                       # placeholder; se actualiza en /confirm
...
async def list_person_photos(
    ...
    return list(await asyncio.gather(*[_photo_to_response(p, db) for p in photos]))
