    ...
    try:
        async with AsyncSessionLocal() as session:
            async with session.begin():
                # RLS en notification_queue requiere rol admin para UPDATE
                await session.execute(
                    text("SET LOCAL aerofinder.current_user_role = 'admin'")
                )
                await session.execute(
                    text(
                        """
...
async def run_notification_worker() -> None:
    ...
    while True:
        try:
            ...
            async with AsyncSessionLocal() as session:
                async with session.begin():
                    # El worker necesita ver todas las filas: fijar rol 'admin'
                    # para que las políticas RLS de alerts y notification_queue
                    # sean satisfechas sin un current_user_id específico.
                    await session.execute(
                        text("SET LOCAL aerofinder.current_user_role = 'admin'")
                    )
