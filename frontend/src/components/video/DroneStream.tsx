...
export function DroneStream({ streamKey, droneId, userRole, className = "" }: DroneStreamProps) {
  ...
  const handleSnapshot = () => {
    ...
    link.download = `aerofinder-snapshot-${Date.now()}.png`;
    link.href = canvas.toDataURL("image/png");
    link.click();
  };

  // ── Acción: Picture-in-Picture ────────────────────────────────────────────
  ...
  useEffect(() => {
    const video = videoRef.current;
    if (!video) return;
    const onLeave = () => { if (isMounted.current) setIsPiP(false); };
  ...
  return (
    <div className={`relative overflow-hidden bg-black ${className}`}>
      ...
      <div className="absolute bottom-0 left-0 right-0 z-10 flex items-center justify-end gap-2 bg-gradient-to-t from-black/70 to-transparent px-2 pb-1.5 pt-4">
        ...
        <button
          onClick={handlePiP}
          className={`rounded p-1 transition-colors ${isPiP ? "text-blue-400 hover:text-blue-300" : "text-white/80 hover:text-white"}`}
          title={isPiP ? "Salir de PiP" : "Picture-in-Picture"}
        >
          <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            <rect x="2" y="2" width="20" height="20" rx="2"/>
            <rect x="11" y="11" width="9" height="7" rx="1" fill={isPiP ? "currentColor" : "none"}/>
          </svg>
        </button>
