    ...
    <div className="mx-auto max-w-4xl px-6 py-8">
      ...
      <div className="mb-6 flex items-start justify-between gap-4">
        ...
        <div className="flex shrink-0 gap-2">
          {(role === "admin" || role === "ayudante" || role === "buscador") && (
            <button
              onClick={async () => {
                try {
                  await personsApi.requestMorePhotos(person.id);
                  // Actualiza timestamp localmente para que el familiar lo vea al recargar
                  setPerson((p) => p ? { ...p, photos_requested_at: new Date().toISOString() } : p);
                } catch (error) {
      ...
      {/* Banner de solicitud de fotos para familiar */}
