...
```bash
./scripts/aerofinder.sh start              # levanta todo, espera health, muestra URLs
./scripts/aerofinder.sh start --rebuild    # igual pero reconstruye imágenes
./scripts/aerofinder.sh status             # estado de contenedores + endpoints
./scripts/aerofinder.sh logs -f            # sigue logs de backend + ai-worker
./scripts/aerofinder.sh logs backend -f    # logs de un servicio específico
./scripts/aerofinder.sh ip 192.168.1.50   # cambia IP pública en .env + DB + rebuild frontend
```
